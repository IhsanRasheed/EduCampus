...
function LeaveAppication() {
  const [letter, setLetter] = useState({ leaveLetter: " ", from: "", to: "" });
  const [leaveHistory, setLeaveHistroy] = useState([]);
  ...
  useEffect(() => {
    ...
    leaveHistoryAPI(headers).then((response) => {
      if (response.data.status) {
        setLeaveHistroy(response.data.leaveHistory);
      }
    });
  }, []);
...
export default LeaveAppication;
