...
const forgot = async (req, res, next) => {
  ...
  try {
    ...
    if (teacherData) {
      ...
      const check = await teacher.updateOne(
        { registerId: data.registerId },
...
const availableMonth = async (req, res, next) => {
  ...
  try {
    ...
    const sortedAvailbleMonth = await availableMonth[0]?.workingDays?.sort(
      (a, b) => b.month - a.month
    );
    res.json({
      status: true,
...
const montlyWorkDays = async (req, res, next) => {
  ...
  try {
    ...
    if (teacherData[0].myBatch !== "") {
      ...
      const workDays = await workingDays[0]?.workingDays?.sort(
        (a, b) => a.month - b.month
      );
      res.json({
        status: true,
...
const addAttendance = async (req, res, next) => {
  ...
  const object = {
    ...
    percent:
      Math.round((data.noOfDaysPresent / data.workingDays) * 100 * 100) / 100,
  };
  try {
    ...
    if (found) {
      ...
      const attendanceArray = await attendanceData[0]?.attendance?.sort(
        (a, b) => b.month - a.month
      );
...
const addStudentMark = async (req, res, next) => {
  const data = req.body;
  const percentage =
    data.subjectMarks.reduce((acc, obj) => acc + obj.mark, 0) /
...
const getMarkDetails = async (req, res, next) => {
  ...
  try {
    ...
    const sortedMarkDetails = markDetails[0].markDetails.sort(
      (a, b) => b.month - a.month
    );
    res.status(200).json({
...
const getAttendanceDetails = async (req, res, next) => {
  ...
  try {
    ...
    const attendanceArray = await attendanceDetails[0]?.attendance?.sort(
      (a, b) => b.month - a.month
    );
    res.json({
      status: true,
